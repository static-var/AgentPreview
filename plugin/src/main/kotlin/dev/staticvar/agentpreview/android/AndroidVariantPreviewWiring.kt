...
internal class AndroidVariantPreviewWiring(
    ...
    override fun wire() {
        classDirsFor(variantName).forEach { classpathArtifact ->
            classpathArtifact.map { it.takeIf { artifact -> artifact.isDirectory || artifact.isFile } }.orNull
                ?: project.logger.info(
                    "AgentPreview inferred Android classpath artifact is absent for variant '$variantName': ${classpathArtifact.get().absolutePath}",
                )
        }
    ...
    private fun runtimeClasspathFor(variantName: String): FileCollection =
        project.files(project.configurations.findByName("${variantName}RuntimeClasspath"))
